...
from ponita.csmpn.data.modules.simplicial_data_debug import SimplicialTransform
...
if __name__ == "__main__":
    ...
    parser.add_argument('--warmup', type=int, default=10,
                        help='number of epochs')
    parser.add_argument('--batch_size', type=int, default=1, #96
    ...
    # Argument to run with filtering the rips lift outputs based on the initial connectivity
    parser.add_argument('--initial_edges', action = 'store_true', help='Use initial connectivity in simplices')
    ...
    if args.simplicial:
        print('args.simplicial is true')
        sim_transform = SimplicialTransform(dim=2, dis=2, label="qm9", init_edges=args.initial_edges, prints=False)
        dataset = QM9(root=args.root, transform=sim_transform)
    else:
        dataset = QM9(root=args.root)
        print('NO simplicial transform.')

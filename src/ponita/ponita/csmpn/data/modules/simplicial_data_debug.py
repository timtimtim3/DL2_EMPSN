...
class SimplicialTransform(BaseTransform):
    ...
    2-simplices."""
    def __init__(self, dim=2, dis:float=2.0, label=None, edge_th=10000., tri_th=10000., molecule_type=None, init_edges=False, prints=False):
    ...
    def __call__(self, graph: Data):
        if self.label=="hulls":
            ...
            if self.molecule_type == "aspirin":
                ...
                x_dict, adj_dict = rips_lift(graph, self.dim, self.dis, filter=True)
                if self.prints:
                    print('x_dict len:', len(x_dict))
                    print('x_dict:', x_dict)
                    print('adj_dict len:', len(adj_dict))
